...
// Check if user is a department advisor or has access to departments
$is_dept_advisor = false;
$user_depts = array();

// Check if user is a department advisor
$advisor_check = mysqli_query($connection, "SELECT dept_id FROM dept_advisor WHERE staffID = '$sxid'");
if ($advisor_check && mysqli_num_rows($advisor_check) > 0) {
    $is_dept_advisor = true;
    while ($row = mysqli_fetch_array($advisor_check)) {
        $user_depts[] = $row['dept_id'];
    }
}

// For staff users who aren't department advisors, show empty state
$has_dept_access = $is_dept_advisor;
...
<head>
    <meta charset="utf-8" />
    <title>CRS | My Department Activities</title>
              ...
              <div class="m-portlet__head-caption">
                <div class="m-portlet__head-title">
                  <h3 class="m-portlet__head-text">
                    ...
                    <?php endif; ?>
                    <?php if (!$has_dept_access): ?>
                      <small class="m--font-warning">(No department access - View only)</small>
                    <?php endif; ?>
                  </h3>
                </div>
              </div>
              <div class="m-portlet__head-tools">
                <ul class="m-portlet__nav">
                  <li class="m-portlet__nav-item">
                    <?php if ($has_active_semester && $has_dept_access): ?>
                    ...
                    <?php else: ?>
                        <button type="button" class="btn btn-accent m-btn m-btn--custom m-btn--pill m-btn--icon m-btn--air" disabled 
                          title="<?php echo !$has_active_semester ? 'No active semester' : 'No department access'; ?>">
              ...
              <table class="table table-striped- table-bordered table-hover table-checkable" id="m_table_1">
                ...
                <tr>
                  ...
                  <td><?php echo $dact_name ?>(<?php echo $dact_id ?>)<br><br>
                    ...
                    ?>
                    <a href="registerStdActivity-b.php?dact_id=<?php echo $token ?>&amp;regpoint=c&amp;resultSearch=<?php echo $resultSearch ?>&amp;regError=<?php echo $regError ?>" title="Add Committee" class="btn btn-primary m-btn btn-sm m-btn m-btn--icon">
                      <span>
                        <i class="fa flaticon-user-add"></i>
                        <span>Committee</span>
                      </span>
                    </a>
                    <a href="registerStdActivity-b.php?dact_id=<?php echo $token ?>&amp;regpoint=p&amp;resultSearch=<?php echo $resultSearch ?>&amp;regError=<?php echo $regError ?>" title="Add Participant" class="btn btn-primary m-btn btn-sm m-btn m-btn--icon">
                      <span>
                        <i class="fa flaticon-user-add"></i>
                        <span>Contestant</span>
                      </span>
                    </a>
                    <a href="registerStdActivity-b.php?dact_id=<?php echo $token ?>&amp;regpoint=a&amp;resultSearch=<?php echo $resultSearch ?>&amp;regError=<?php echo $regError ?>" title="Add Audience" class="btn btn-primary m-btn btn-sm m-btn m-btn--icon">
                      <span>
                        <i class="fa flaticon-user-add"></i>
                        <span>Audience</span>
                      </span>
                    </a>
                    <a href="RegisteredListD.php?dact_id=<?php echo $token ?>&amp;resultSearch=<?php echo $resultSearch ?>&amp;regError=<?php echo $regError ?>" title="Overall List" class="btn btn-info m-btn btn-sm m-btn m-btn--icon">
                      <span>
        ...
        <div class="modal-dialog modal-lg" role="document">
          <div class="modal-content">
            ...
            <div class="modal-body">
              <form action="createDeptActivity.php" method="post">
                ...
                <div class="form-group">
                  ...
                  <select class="custom-select form-control" name="dept_id" <?php echo (!$has_active_semester || !$has_dept_access) ? 'disabled' : ''; ?> required>
                    <option value="">Select Department</option>
                    <?php
                    if ($has_dept_access) {
                        $sql_depts = mysqli_query($connection, "SELECT d.* FROM dept d 
                          JOIN dept_advisor da ON d.dept_id = da.dept_id 
                          WHERE da.staffID = '$_SESSION[username]' 
                          ORDER BY d.dept_name");
                        
                        if (mysqli_num_rows($sql_depts) > 0) {
                            while ($dept_row = mysqli_fetch_array($sql_depts)) {
                                $dept_id = $dept_row['dept_id'];
                                $dept_name = $dept_row['dept_name'];
                                echo "<option value=\"$dept_id\">$dept_name</option>";
                            }
                        } else {
                            echo "<option value=\"\" disabled>No departments assigned to you</option>";
                        }
                    } else {
                        echo "<option value=\"\" disabled>You are not assigned as a department advisor</option>";
                    }
                    ?>
                  </select>
                  <?php if (!$has_dept_access): ?>
                    <small class="form-text text-muted">Only department advisors can create activities. Please contact administrator if you need department access.</small>
                  <?php endif; ?>
                </div>
            ...
            <div class="modal-footer">
              <span class="m--font-danger">Note : Only Mentor/Mentee activities will be approved automatically. Others will need approval from Moderators</span>
              <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
              <button type="submit" class="btn btn-primary" <?php echo (!$has_active_semester || !$has_dept_access) ? 'disabled' : ''; ?>>Save</button>
            </div>
              </form>
          </div>
        </div>
